...
def activate_user():
    if request.method == 'POST':
        ...
        if result == None:
            cur.execute('SELECT role_name FROM roles WHERE role_name=%s;', (role,))
            try:
                res = cur.fetchone()
            except ProgrammingError:
                res = None
...
def update_transit():
        if request.method == 'POST':
                ...
                if action=='Load':
                        ...
                        #           (SELECT transit.destination_fk FROM transit WHERE unload_dt=%s AND WHERE asset_fk=%s), %s)', (asset_fk, action_dt, asset_fk, action_dt))
                        conn.commit()
                        cur.close()
                        conn.close()
                        return render_template('dashboard.html')
                ...
                if result != ('Logistics Officer',):
                        ...
                        cur.execute('SELECT asset_tag FROM assets JOIN transit ON assets.assets_pk=transit.asset_fk WHERE transit.unload_dt IS NULL;')
                        res = cur.fetchall()
                        transit_tag = [] 
                        for r in res:
                                row=dict()
