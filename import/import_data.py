	...
	with open("transfers.csv") as f:
		...
		for s in transfers:
			...
			print("""INSERT INTO transit (transfer_fk, asset_fk, load_dt, unload_dt) VALUES ((SELECT transfer_pk FROM transfer WHERE asset_fk=(SELECT assets_pk FROM assets WHERE asset_tag='{}') 
				AND request_dt=(SELECT max(request_dt) FROM transfer WHERE asset_fk=(SELECT assets_pk FROM assets WHERE asset_tag='{}'))), 
			...
			print("""INSERT INTO asset_at (asset_fk, facility_fk, arrive_dt) VALUES ((SELECT assets_pk FROM assets WHERE asset_tag='{}'), 
				(SELECT facilities_pk FROM facilities WHERE fcode='{}'), '{}');""".format(s[0], s[6], s[8]))
